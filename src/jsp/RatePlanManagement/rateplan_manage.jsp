          ...
          <%@ include file="../lang.jsp" %>
				...
				if(( chargecode != null || pageId != -1) && chargecode.size()!=0)    
				{
								...
								%>
												...
												{
																%>
																				<a href="rateplan_manage.jsp?pid=<%=pageId-1%>&keywords=<%=keywords%>&srchtext=<%=searchtext%>&order=<%=order%>&searchId=0">&lt;&lt; <%=TSSJavaUtil.instance().getKeyValue("previous",defLangId)%></a> &nbsp;&nbsp;
																<%
												}
												else
												{
								...
								%>
												...
												{
																%>
																				&nbsp;&nbsp;<a href="rateplan_manage.jsp?pid=<%=pageId+1%>&keywords=<%=keywords%>&srchtext=<%=searchtext%>&order=<%=order%>&searchId=0"><%=TSSJavaUtil.instance().getKeyValue("next",defLangId)%> &gt;&gt;</a>
																				<%
												}
												else
												{
