...
<%
    Logger logger = Logger.getLogger ("modifyprepost.jsp");
...
String user_name=sessionHistory.getUser();
    PrePostManager serviceManager = new PrePostManager();
     serviceManager.setConnectionPool(conPool);
				...
				if(k==1)
				{
								logobj.setTableName("SERVICE_CLASS_RANGE");
								logobj.setlink("prepostclasslog");
				...
				int i = serviceManager.updatePrePost(serviceDetail,rangeId);
if(i>=0 && k==1)
{
...
logger.info("logs return =="+res);
				...
				if(i < 0)
				{
								if(i == -2 || i==-3 || i==-4)
								{
												logger.info("webadmin/PrepostManager: Modified Range is overlapping with someother Existing Range, please enter another");
												%>
