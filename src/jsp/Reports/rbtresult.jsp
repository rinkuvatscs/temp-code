			...
			else
			{
				...
				if( (format.equalsIgnoreCase("Tabular")) || (format.equalsIgnoreCase("Line Graph")) || (format.equalsIgnoreCase("Bar Graph")) )
				{
					...
					%>	
						...
						{
							%>
								<a href="rbtresult.jsp?reportId=<%=reportId%>&start=<%=start%>&end=<%=end%>&&aggre=<%=aggre%>&format=<%=format%>&pageId=<%=(pageId-1)%>&reportType=<%=reportday%> &contentp=<%=cpCode%>">  &lt;&lt; <%=TSSJavaUtil.instance().getKeyValue("previous",defLangId)%></a>
								<%
						}
					if(pageCount > 1 && pageId < pageCount)
					{
						%>
							&nbsp;&nbsp;&nbsp;
						<a href="rbtresult.jsp?reportId=<%=reportId%>&start=<%=start%>&end=<%=end%>&&aggre=<%=aggre%>&format=<%=format%>&pageId=<%=(pageId+1)%>&reportType=<%=reportday%>&contentp=<%=cpCode%> "><%=TSSJavaUtil.instance().getKeyValue("next",defLangId)%> &gt;&gt; </a>
							<%
					}
					%>
						</td></tr>
