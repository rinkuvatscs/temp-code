...
tempId=Integer.parseInt(searchtext);
    	...
    	i = smsManager.getHelpTemplateSMS (templateSmsAl, language, searchtext,orderby,searchId);   
...
Search by:
     <select name="srchId" class="formField" onChange="enbdes()">
               ...
               <option value=1>SMS Keyword</option>
               <option value=2>Help SMS</option>
   </select>
</td>
...
<tr>  <td>
      <table width="100%" border="1"  cellpadding="2" cellspacing="2">
			<tr class="tfields" bgcolor="#a5c7d0">
				<td width="30%" ><a href="smsTemplate.jsp?pid=<%=pageId%>&languageId=<%=language%>&ordrby=2&srchId=<%=searchId%>&srchtext=<%=searchtext%>">SMS Keyword</a></td>
				<td width="55%"><a href="smsTemplate.jsp?pid=<%=pageId%>&languageId=<%=language%>&ordrby=1&srchId=<%=searchId%>&srchtext=<%=searchtext%>">Help SMS</a> </td>
		...
		<%if ( (r % 2) == 0){ %>class="rowcolor1"<%} else {%> class="rowcolor2"<%}%> >
			...
			<td > <a href="smsTemplate_view.jsp?tkey=<%=((TemplateSMS)templateSMSArr[r-1]).getTemplateDescription() %>&languageId=<%=language%>"><img src="../images/view.gif" height="20" border="0"></a> </td>
		<%
		if( sessionHistory.isAllowed(301))
		{
		%>
			<td ><a href="smsTemplate_modify.jsp?tkey=<%=((TemplateSMS)templateSMSArr[r-1]).getTemplateDescription() %>&languageId=<%=language%>"><img src="../images/modify.gif" height="20" border="0"></a> </td>
		<%
		}
		%>          
		</tr>
...
<tr class="homemenu"><td></td><td><a href="../SMSTemplate/home.jsp">Back</a></td></tr>
</td></tr>
</table>
